...
右键浏览器工具栏中的 **JavBoss 助手** 图标，选择 **选项**，或在 Chrome 扩展管理页的扩展详情中点击 **扩展程序选项**，即可打开设置。选项页与工具栏弹窗共用同一套连接设置和功能开关，修改后自动保存。
...
## 影片拥有状态（JavDB 0.12.0 起，JavBus / JavLibrary 0.13.0 起）

扩展 0.14.0 起，可在工具栏弹窗的 **显示已拥有状态** 开关中控制展示，默认开启。关闭后立即移除已打开页面的标记并停止后续拥有状态查询；重新开启后自动查询并展示，无需刷新页面。此开关独立于连接设置和磁力下载。

配置扩展的 **连接设置** 后，JavDB、JavBus 列表页在封面右上角，JavLibrary 列表页在卡片容器右上角，三站详情页在番号栏仅为已拥有影片显示 **已拥有**，未拥有影片不显示标记，无需启用磁力下载。搜索、女优、系列等使用相同影片卡片的列表及动态加载卡片均适用；支持 `https://javdb.com`、`https://www.javbus.com` 和 `https://www.javlibrary.com`。JavDB、JavBus 的标记向上溢出自身高度的 50%；JavLibrary 标记位于卡片右上角内部，不溢出。

“已拥有”表示影片关联了未删除的视频文件记录，且所在目录未删除；目录是否启用不影响拥有状态，暂时离线的目录也算拥有。只有元数据或收藏记录不算拥有。番号匹配忽略大小写、空格、连字符和下划线，并使用完整番号匹配。

只有查询成功且确认已拥有后才插入标记；查询中、未拥有或查询失败时均不显示标记。连接异常可在扩展的连接设置中测试连接，修正后刷新页面重试。更新连接设置会自动刷新；离开标签页超过 30 秒后返回会重新查询，手动刷新页面也可更新状态。需要同时更新后端和扩展。
...
远程 Server 必须使用 HTTPS（例如 `https://javboss.example.com`），本机 `localhost`、`127.0.0.1`、`[::1]` 可使用 HTTP。支持地址包含反向代理子路径。连接设置只保存当前的 JavBoss 地址和 API 令牌；修改地址不会切换或清空令牌，不再保存历史服务器配置。

Token 是浏览器扩展的接口凭据，不区分 scope；后台按请求方法和路由配置哪些接口接受 Token，目前开放 `POST /extension/downloads`、`GET /extension/status`（连接及鉴权检查）和 `POST /extension/jav/ownership`（批量拥有状态）。后台支持查看有效期、最近使用时间、重新生成及删除。重新生成后旧 Token 立即失效；删除某个 API 令牌不会影响其他浏览器。到期或删除后，扩展会提示重新配置 Token。

扩展将 Token 保存在仅可信扩展上下文可访问的本地存储，不进行 Chrome Sync 同步。service worker 提交下载或查询拥有状态、扩展设置页测试连接时会向指定 Server 发送 Token；页面内容脚本仅接收功能状态和所查询番号的拥有状态，不接收地址、Token 或视频文件路径。扩展不会跟随这些接口的 HTTP 重定向，Server 地址应填写最终地址。
...
JavBoss 助手的固定扩展 Origin 只可对已配置的接口发起跨域请求；`OPTIONS` 无需 Token，目前允许 `POST /extension/downloads`、`GET /extension/status` 与 `POST /extension/jav/ownership`，请求头允许 `Content-Type`、`Authorization`。无 Origin 的客户端使用 Bearer Token 时也受相同接口配置限制。网页 Cookie 登录保留原有来源校验。
...
`POST /extension/jav/ownership` 接收 JSON `{"codes":["ABC-123","082226_01"]}`，一次 1–200 个番号，每个最多 128 字节，整个请求体最多 32 KiB，无查询参数。返回 `{"items":[{"code":"ABC-123","owned":true},{"code":"082226_01","owned":false}]}`，只包含所查询番号及布尔状态；响应禁止缓存。番号支持英文字母、数字、空格、点、连字符和下划线，前后空白会去除。
