...
- 集成 CloudDrive2 grpc 接口，配合自带的 Chrome 扩展可实现任意网站点击磁力链接直接下载到本地。
...
Docker 部署下默认只能使用浏览器播放器（维护力度较弱只能保证基本可用性），可参考[Client 模式说明](#client-模式说明)以使用 MPV 播放器获得更好的播放体验。添加目录时直接填写宿主机路径，例如 `/mnt/disk1/videos`，程序会自动映射到容器内可访问路径。
